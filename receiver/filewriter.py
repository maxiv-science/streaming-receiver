...
class FileWriter:
    ...
    def run(self, worker_queue: Queuey, writer_queue: Queuey):
        self._thread = Thread(
            target=self._main, args=(worker_queue, writer_queue), daemon=True
        )
        self._thread.start()
        ...
        try:
            ...
            if filename and saveraw:
                if os.path.isfile(filename):
                    ...
                    logger.info(
                        "h%d: opened existing file %s to append, isfile took %lf, with open took %lf",
                        msg_number,
                        filename,
                        time_isfile,
                        time_opened,
                    ...
                    logger.info(
                        "h%d: created new file %s with dataset %s, open took %lf, with created took %lf",
                        msg_number,
                        filename,
                        group_name,
                        time_opened,
            ...
            status = {"htype": "status", "state": "running"}
            ...
            status = {"htype": "status", "state": "error", "error": str(e)}
    ...
    def _handle_end(self, header, worker_queue):
        ...
        if self._fh:
            self._fh.close()
        status = {"htype": "status", "state": "idle"}
